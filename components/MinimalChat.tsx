...
export function MinimalChat() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = data?.text || data?.content || "Sorry, I couldn't process that request.";

      // Simulate streaming by updating the message content
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantMessageId ? { ...msg, content: responseText, isStreaming: true } : msg
        )
      );
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen pt-20 pb-6">
      ...
      <div className="text-center space-y-4 mb-8 px-6">
        ...
        <p className="text-muted-foreground text-lg font-medium">Your intelligent AI companion</p>
      </div>
        ...
        <CardHeader className="pb-4 bg-gradient-to-r from-primary/5 to-secondary/5 border-b border-primary/10 flex-shrink-0">
          <CardTitle className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-semibold">Live Conversation</span>
            </div>
            {messages.length > 0 && (
              <Button
